...
- `src/distill.py` — DAgger distillation to migrate a trained teacher onto a fresh student across an **architecture or observation-layout change** without a curriculum restart (the general case `src/widen_checkpoint.py` can't cover — that one only handles same-obs function-preserving widening). The student is regressed onto the teacher's deterministic mean action **and** value (the value must be distilled too — a warm actor with a fresh critic is the classic PPO-resume trap) on states an annealed teacher→student rollout visits under the student's own deployment DR. Two teacher-obs regimes via `distill.teacher_obs`: `identical` (teacher sees the student's obs — architecture-only changes) and `privileged` (teacher sees `SO101BaseEnv.privileged_obs()`, the always-fresh-tag view, while the student trains on camera obs with dropout — imitation actively teaches the student to infer hidden tag pose). `log_std` is copied, not regressed. Reuses `src/train.py`'s `env_specs` / `obs_norm_for` / `build_fresh_model` so the student's env farm, input normalization, and architecture match training exactly. Output is a `resume`-compatible checkpoint; **always follow distillation with a short PPO fine-tune** (`python -m src.train resume=…`). `privileged` mode currently requires `frame_stack=1`. Config in the `distill:` block of `conf/config.yaml`; contract-tested in `tests/training/test_distill.py` and `tests/env/test_privileged_obs.py`.
...
python -m src.train env=lift resume=distilled.zip                             # mandatory fine-tune
```
