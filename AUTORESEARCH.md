...
3. **Read the in-scope files** for full context:
   ...
   - `lift_env.py` — lift task env (reward function, reset logic, success criteria)
...
Training runs with a **fixed time budget of 10 minutes** (configured in `conf/env/lift.yaml`). Launch:
...
This prints the average `rollout/lift/mean_max_cube_height` over the last minute of training.

**What you CAN modify:**
- `lift_env.py` — reward shaping, observation space, reset logic, success criteria
...
- `conf/env/lift.yaml` — lift-specific config (action scale, max steps, substeps, cube spawn range, penalties)
...
**The goal: maximize `mean_max_cube_height`.** The cube starts on the table at z≈0.015. The target is 0.10 (10cm lift). Higher is better. Everything is fair game: reward shaping, observation design, hyperparameters, network architecture, action scale, physics parameters.
...
```
commit	mean_height	timesteps_M	status	description
```

1. git commit hash (short, 7 chars)
2. mean_max_cube_height over last minute (e.g. 0.0310)
...
```
commit	mean_height	timesteps_M	status	description
a1b2c3d	0.0310	2.5	keep	baseline
b2c3d4e	0.0450	2.4	keep	increase action_scale to 0.1
c3d4e5f	0.0280	2.6	discard	switch to SAC
d4e5f6g	0.0000	0.0	crash	double net width (OOM)
```
...
4. Run: `conda run -n mujoco_env python train.py env=lift > run.log 2>&1`
   ...
   - **Timeout**: Each run should take ~10 minutes. If it exceeds 15 minutes, kill it and treat as failure.
...
8. If mean_height improved → keep the commit, advance the branch.
9. If mean_height is equal or worse → `git reset --hard HEAD~1` to revert.
