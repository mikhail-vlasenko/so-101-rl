...
- **Correlated marker dropout unmodeled.** When the real pipeline loses the *table* tag it zeroes BOTH arm tags at once (`real/marker_obs.py` can't re-anchor the camera), while sim dropout (`marker_dropout`) is rolled independently per tag — add a correlated all-tags dropout mode if real logs show it matters. (Camera latency itself is done: `src/camera_sim.py` + `sysid/probe_cam_latency.py`, measured `delay_ms: [42, 52]` in `conf/dr`.)
