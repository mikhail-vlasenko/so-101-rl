...
- **Marker observations: calibrate sim sites against the real glued tags; wire the camera into rollouts.** The policy obs now carries world poses of `marker_finger` / `marker_wrist` sites (`so101.xml`). Their placement is an eyeballed estimate of where the tags were glued (finger site assumes "bottom of fixed jaw", wrist site assumes "right = -y world when the arm faces the workspace") — measure the real tag poses (e.g. via the digital twin + camera at a known joint pose) and correct the site pos/quat. Separately, `real/rollout_lift.py` currently fills the marker obs from FK on the lockstep sim, not from the camera; until the AprilTag pipeline feeds measured poses there, the marker obs adds no real-world information. When wiring it: an undetected tag must yield exactly zeros for its 6 obs dims (the sim convention — `base_env._compute_obs` zeros tags turned >70° from the `tag_cam` camera in `so101.xml`, plus a stochastic detector dropout near the boundary), and `tag_cam`'s pose should be measured from the real mount, not eyeballed. The sim visibility check is plane-angle only (no FOV/occlusion test); the stochastic miss is the `dr` group's `marker_dropout` (per-frame drop prob, higher in the grazing 65–70° band) — tune both against measured real-camera dropout rates. There is deliberately no reward term for a hidden tag: losing its pose is penalty enough.
