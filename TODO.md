...
- **Cube tag yaw uncalibrated.** The cube obs is the raw pose of tag id 1 on the sponge's largest face; sim assumes the tag frame is axis-aligned with the cube body, but the real tag's yaw about its own normal (how it's rotated on the face) has never been measured. Calibrate it (place the sponge in a known orientation, snap the in-plane offset like `quarter_turns` does for the arm tags) and either un-rotate in `real/marker_obs.py` or randomize the yaw in sim. Doesn't affect the tag position or the derived center height (offset runs along the normal), only the rot channel's usefulness.
