...
- **Redo sysid; stop sharing one actuator class across all servos.** The fit behind the `sts3215` class in `so101.xml` (`kp=77.5 kv=2.731`) was done with all servos at Kp 32, but the real arm now runs per-joint gains (`SERVO_POSITION_KP` in `real/twin/constants.py`: pan and gripper at 8, rest at 32), so the sim's pan actuator is ~4x stiffer than real. One shared class can't represent per-joint servo settings — give each joint (or at least each Kp group) its own actuator class and refit kp/kv per joint against step responses recorded at the current servo settings.
