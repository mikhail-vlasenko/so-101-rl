...
- **Add load-dependent joint deflection (linear gravity compliance) to the qpos calibration and deployment.** After the mount write-back, the calibration residual is still structured and gravity-correlated: extending the solve with `q_true = q_enc − b − c_i·τ_grav,i` (lift/elbow/wrist_flex) cuts held-out error from mean 3.8 / max 8.1 mm to mean 2.1 / max 5.3 mm (even/odd pose split on the 2026-07 samples). The backlash probe (`sysid/probe_backlash.py`, data in `sysid_logs/backlash_records.json`) ruled out gear play as the mechanism: link-vs-motor hysteresis is ≤0.2° on every joint (pan/roll at τ≈0 bound the true gap, so the term is *elastic* flex — linear at the wrist too, no backlash step needed). Deployment side: correct the qpos obs by `c·τ` using the lockstep sim's `qfrc_bias`; note gravity-only τ is wrong during contact — modeling the compliance in sim (springy `*_play` joints) is the principled alternative if that matters. Related sim cleanup: the `backlash` joint class's ±0.5° is an overestimate (measured ≤0.2°). Separate probe fact: arriving at the same target from opposite directions leaves the *motor* up to 2.1° apart at pan (stiction vs kp=8; enc Δ tracks the per-joint kp ordering) — the encoder sees it so obs stay truthful, but open-loop moves land 1–2° off depending on approach direction.

- **Marker observations: tune sim dropout against measured real rates; site rotations still eyeballed.** Mostly done: the camera feeds measured tag poses into rollouts (`real/marker_obs.py`, `--marker-source camera`, zeroing undetected tags like training), the site *positions* are calibrated (`real/calibrate_qpos.py` writes the freed-mount solve back into `so101.xml`), and `tag_cam`'s sim pose is set from the solved extrinsics. Still open: the site *rotations* are an eyeballed estimate plus the `quarter_turns` in-plane snap — fine while the obs uses positions, revisit if marker_rot ever matters; and the sim visibility model (plane-angle cutoff + the `dr` group's `marker_dropout`, higher in the grazing 65–70° band) has never been checked against measured real-camera dropout rates. There is deliberately no reward term for a hidden tag: losing its pose is penalty enough.
